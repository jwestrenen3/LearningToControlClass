...
# ╔═╡ 501fc67d-c1f7-4972-afe7-1fd83d7c541a
using PlutoUI, LinearAlgebra, ControlSystems, Random, Plots, ControlSystems

# ╔═╡ 8001fe2a-47b5-419b-aae8-057d86ed0db3
# ---------------------------
# 1. System parameters
# ---------------------------
begin
...
# ╔═╡ 5a4e1786-7619-4a46-940f-cb2a614edced
md"""
# Question 1:

How might abrupt changes in setpoint affect controller performance?
"""

# ╔═╡ cc840e10-100c-42d9-bc26-8ffc0d3cea06
# ---------------------------
# 3. Setpoint profile
# ---------------------------
begin
	...
	x_set[Int(N*0.6):end] .= 50.0
end

# ╔═╡ 8f70dff7-b120-4f0e-aa6f-3bff5fb93d5b
# ---------------------------
# 3. Disturbances
# ---------------------------
begin
	...
	w_structured = 2 .* sin.(0.5 .* (0:N-1))         # structured, persistent
	w_noise = 2.0 .* randn(N)                        # small Gaussian
	w_process = w_structured .+ w_noise

	v_meas = 15 .* randn(N)                          # measurement noise

	plot(w_process, label="Process disturbance (structured + noise)")
end


# ╔═╡ 3c4dde35-5278-4450-8759-b41c3fc03200
md"""
# Question 2:

Why would a standard LQG controller struggle with this type of disturbance?
"""

# ╔═╡ deadeb0b-a996-476e-b5a4-de91b4369b2d
# ---------------------------
# 5. LQR gain calculation
# ---------------------------
begin
	# Define system matrices as 1×1 matrices
	A = [-0.3]   |> x -> reshape(x, 1, 1)
	B = [1.0]    |> x -> reshape(x, 1, 1)
	Q = [1.0]    |> x -> reshape(x, 1, 1)
	R = [0.1]    |> x -> reshape(x, 1, 1)

	# Solve continuous-time ARE
	P = care(A, B, Q, R)

	# Compute LQR gain (scalar)
	K_lqr = (B' * P / R)[1]
end

# ╔═╡ fae71147-b0a0-4d6d-bca6-5bb90bf3d436
# ---------------------------
# 6. H∞ gain
# ---------------------------
begin
...
# ╔═╡ e760331b-109d-4436-b06a-ad2a65655794
# ---------------------------
# 7. Simulation setup
# ---------------------------
begin
...
# ╔═╡ 37e5d2c6-4599-44cb-ba4d-346829ff2a8d
# ---------------------------
# 8. Simulation loop
# ---------------------------
begin
...
# ╔═╡ c23b3626-2083-421a-aa5d-d726db535148
# ---------------------------
# 9. Plot results
# ---------------------------
begin
	plot(
		...
		title="LQG vs H∞: Persistent disturbance scenario",
		grid=:both
	)
	plot!(x_lqg, label="LQG")
	plot!(x_hinf, label="H∞ Controller")
end


# ╔═╡ 062ce691-dc2c-427f-bf2c-58635faaf672
md"""
# Question 3:

What makes H∞ control "robust" compared to LQG in the presence of persistent disturbances?
"""
...
manifest_format = "2.0"
project_hash = "30507bf06d65f37b791c9cf808240cd130dc399a"
